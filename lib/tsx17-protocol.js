		...
		self.tsx17.sendPollingAndWait(function(error, ret) {
			if (error) {
				...
				if (error.code === 'TIMEOUT') {
					...
					self.tsx17.reset(function(error) {
						if (error) {
			...
			if (ret === EOT) {
				debug("_sendCommand.waitResponse: get EOT, wait another response");

				return waitResponse();
			}

			if (ret === NACK) {
				debug("_sendCommand.waitResponse: get NACK, retry command");
			...
			debug("_sendCommand.waitResponse: ", ret);
