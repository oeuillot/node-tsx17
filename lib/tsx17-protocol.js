...
TSX17_protocol.prototype.sendCommand = function(command, callback) {
	var tryCount = 0;
	...
	function sendCommand() {
		...
		self.tsx17.sendPacket(config, command, function(error) {
			if (error) {
				return callback("Send packet error: " + error);
			}

			// console.log("Packet sent, waiting response ...");

			self.tsx17.waitResult(function(error, response) {
				if (error && error.code !== 'TIMEOUT') {
					return callback("Wait response error: " + error);
				}
				if (response === ACK) {
					return waitResponse();
				}
				if (response === NACK) {
					console.log("NACK retry");
					return sendCommand();
				}
				console.log("Unknown return code ", response);
				return sendCommand();
			});
		});
	}
...
TSX17_protocol.prototype.readInfos = function(callback) {
	this.sendCommand([ 0x02, 0x03, 0x00, 0x00 ], function(error, ret) {
		if (error) {
			return callback(error);
		}
...
TSX17_protocol.prototype.readDatas = function(type, offset, count, callback) {
	var buffer = [];
	buffer[0] = 0x36;
	buffer[1] = this.config.categorie;
	buffer[2] = type;
	...
	this.sendCommand(buffer, function(error, ret) {
		if (error) {
			return callback(error);
		}
...
TSX17_protocol.prototype.readWords = function(offset, count, callback) {
	...
	// console.log("ReadWords offset=", offset, "count=", count);

	this.readDatas(0x68, offset, cnt, function(error, ret) {

		// console.log("ReadDatas RETURN error=", error, " ret=", ret);
		...
		// console.log("ReadDatas array=", r);

		if (cnt === count) {
			return callback(null, r);
		}

		self.readWords(offset + cnt, count - cnt, function(error, ret2) {
			if (error) {
				return callback(error);
			}
...
TSX17_protocol.prototype.setWords = function(words, offset, count, callback) {
	...
	this.sendCommand(buffer, function(error, ret) {
		...
		self.setWords(newWords, offset + cnt, count - cnt, callback);
	});
