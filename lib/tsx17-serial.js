...
TSX17.prototype.tryBauds = function (bauds, resetBauds, callback) {
	...
	const sp = new Serialport(this.portName, {
		baudRate: bauds,
		dataBits: 8,
		stopBits: 1,
		parity: 'odd',
...
TSX17.prototype.sendPollingAndWait = function (callback) {
	...
	const now = Date.now();
	if (this.$lastTime && this.$lastTime + DELAY_WAITING_MS > now) {
		const dt = this.$lastTime + DELAY_WAITING_MS - now;
		this.$lastTime = now;
//		console.log('Wait ', dt);
		setTimeout(() => {
			this.sendPollingAndWait(callback);
		}, dt);
		return;
	}
