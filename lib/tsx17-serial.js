...
TSX17.prototype.tryBauds = function(bauds, resetBauds, callback) {
	...
	console.log("Open serial port " + this.portName + " bauds=" + bauds + " reset=" + resetBauds);
	...
	function dataListener(buffer) {
		...
		if (poolingRetry < 5) {
			...
			console.log("Retry pooling #" + poolingRetry);
		...
		self.resetBauds(function(error) {
			...
			// console.log("Reset Bauds command sent !");
...
TSX17.prototype.sendPollingAndWait = function(callback) {
	...
	console.log("Send PollingBuffer");
	var self = this;
	this.serialWrite(POLLING_BUFFER, function(error) {
		...
		serialPort.drain(function(error) {
			...
			self.waitResult(function(error, packet) {
				...
				console.log("Wait result=", packet);
...
TSX17.prototype.open = function(callback) {
	...
	function try19200(callback) {
		self.tryBauds(19200, false, function(error, sp) {
			if (error) {
				console.log("Try 19200 returns ", error);
			...
			self.tryBauds(9600, false, function(error, sp) {
				if (error) {
					console.log("Try 9600 returns ", error);
					if (error.code !== "TIMEOUT") {
						return callback(error);
					}
				}
				...
				self.tryBauds(300, true, function(error, sp) {
					if (error) {
						console.log("Try 300 returns ", error);
						if (error.code !== "TIMEOUT") {
							return callback(error);
						}
					}
					...
					self.tryBauds(9600, false, function(error, sp) {
						if (error) {
							console.log("Retry 9600 returns ", error);
							if (error.code !== "TIMEOUT") {
								return callback(error);
							}
						}
...
TSX17.prototype.installPort = function(sp, callback) {
	...
	sp.on("data", function(buffer) {
		...
		console.log("Read data from serial ", buffer, " bufSize=", self.stream.bufferSize);
	});
...
TSX17.prototype.waitResult = function(callback) {
	...
	console.log("Wait result ! bufferSize=" + this.stream.bufferSize);
	...
	function readSTX() {
		...
		if (buf[0] !== STX) {
			return callback("INVALID DLE/STX protocol");
		}
	...
	for (;;) {
		...
		console.log("ProcessData input=", buf);

		if (!buf) {
			console.log("Buf is empty size=" + this.stream.bufferSize);
			break;
		}

		var b = buf[0];

		console.log("First packet byte " + b);
		...
		console.log("Unknown code ", buf, " wait another ...");
	}

	var timeoutId = setTimeout(function() {
		self.stream.removeListener('bufferReady', waitData);
		var error = new Error('WaitResult Timeout');
		error.code = "TIMEOUT";
...
TSX17.prototype.processDLE = function(callback) {
	...
	if (len < 0) {
		this.stream.once('bufferReady', this.processDLE.bind(this));
		return;
	}
	...
	function loadContent() {
		...
		// console.log("Process DLEPacket bcc=", bcc, "Buf=", buf);
